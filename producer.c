...
int main(void) {
  ...
  if (!read_entire_file(csv_path, &csv_content))
    return 1;
  ...
  retry:
    err = rd_kafka_producev(rk, RD_KAFKA_V_TOPIC(kafka_config.topic),
                            RD_KAFKA_V_MSGFLAGS(RD_KAFKA_MSG_F_COPY),
                            RD_KAFKA_V_VALUE(msg, len), RD_KAFKA_V_OPAQUE(NULL),
                            RD_KAFKA_V_END);

    if (err) {
      fprintf(stderr, "%% could not produce to topic %s: %s\n",
              kafka_config.topic, rd_kafka_err2str(err));
