...
rd_kafka_conf_t *set_configs(Kafka *conf, char *err_str)
{
    ...
    if (rd_kafka_conf_set(kafka_config, "bootstrap.servers",
                          conf->brokers,
                          err_str,
    ...
    if (rd_kafka_conf_set(kafka_config, "group.id",
                          conf->group_id,
                          err_str,
                          sizeof(err_str)) != RD_KAFKA_CONF_OK)
    {
    ...
    if (rd_kafka_conf_set(kafka_config, "auto.offset.reset",
                          "earliest",
                          err_str,
                          sizeof(err_str)) != RD_KAFKA_CONF_OK)
    {
    ...
    if (rd_kafka_conf_set(kafka_config, "enable.auto.commit", "true", err_str, sizeof(err_str)) != RD_KAFKA_CONF_OK)
    {
...
int main()
{
    ...
    rd_kafka_t *rk = rd_kafka_new(RD_KAFKA_CONSUMER, conf, err_str, sizeof(err_str));
    ...
    rd_kafka_topic_partition_list_t *subscriptions = rd_kafka_topic_partition_list_new(TOPICS_COUNT);
    for (int i = 0; i < TOPICS_COUNT; ++i)
    {
        rd_kafka_topic_partition_list_add(subscriptions, kafka_config.topics[i], RD_KAFKA_PARTITION_UA);
    }
    ...
    {
        fprintf(stderr,
                "Could not subscribe to %d topics: %s\n",
